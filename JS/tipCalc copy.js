    ...
    constructor() {
        ...
        this.calculate = () => {
            ...
            if (this.inputCustom.value !== "") {
                const totalPerPerson = (Number(this.bill) + Number(this.bill) * (Number(this.custom) / 100)) /
                    Number(this.numberOfPersonss);
                const tipAmount = (Number(this.bill) * (Number(this.custom) / 100)) /
                    Number(this.numberOfPersonss);
        ...
        this.listeners = () => {
            ...
            this.tips.forEach((tip) => tip.addEventListener("click", () => {
                ...
                const totalPerPerson = (Number(this.bill) + Number(this.bill) * Number(tip.value)) /
                    Number(this.numberOfPersonss);
                const tipAmount = (Number(this.bill) * tip.value) / Number(this.numberOfPersonss);
                this.notANumber(totalPerPerson, tipAmount);
            }));
        };
        this.reset = () => {
